...
rf2 <- randomForest(train_prepared[,-c(1,7:9,17:19)],train_prepared$high.end,
                     sampsize=round(0.6*length(train_prepared[,20])),
                    ntree=500,mtry=sqrt(16),importance=TRUE)
rf2
importance(rf2)
# Again neighborhood and building class category are among the most important, along with gross square footage, sale day, sale month, building age, block, and lot
...
rnd.train.pred<-predict(rnd.class,newdata=train_prepared) 

CrossTable(train_prepared$high.end, rnd.train.pred,
           prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
           dnn = c('actual diagnosis', 'predicted diagnosis')) #0.963 accuracy
...
boost2 <- boosting(high.end ~ ., data=train_prepared, boos = FALSE, mfinal=50)
...
boostpred <- predict(boost2,test_prepared)
boostpreds <- as.data.frame(boostpred$class)
write.csv(boostpreds, "boostpredictions.csv")
