...
# high end and tax class should be categorical
...
```
We got rid of BOROUGH, EASE.MENT, ADDRESS because BOROUGH variable has unanimous values (always 1), all the EASE.MENT values are NA, and ADDRESS will not be a primary predictor - the street name could potentially be useful but . We also factorized high.end, TAX.CLASS.AT.TIME.OF.SALE, RESIDENTIAL UNITS,and COMMERCIAL UNITS. Now, we implemented the same data process to the test training set. 
